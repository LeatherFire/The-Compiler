...
const PopupFavorities = ({ id, onClose, onMain}) => {
  const [size, setSize] = useState({ width: 390, height: window.innerHeight*(0.79) });
  const [position, setPosition] = useState({ x: 520, y: 65 });
  ...
  return (
    <Rnd
      onMouseDown={() => handleClick("favorities")}
      onClick={() => handleClick("favorities")}
      style={{ zIndex: zindex["favorities"] }}
      id="popup"
      size={size}
      position={position}
    ...
    >
      <div className="handle items-center border-b-[1px] justify-between border-gray-400 rounded-t-xl backdrop-blur-lg titlebar font-bold bg-gray-100 bg-opacity-80" style={{ height: '28px', width: '100%' }} id="titlebar">
        ...
        <div className='flex justify-center'>
          <p className="leatherfire flex-shrink-0 pt-1 text-gray-700 font-mono text-[9pt] items-center flex gap-1 justify-center">
            <span className="relative shadow-2xl"><MdFavorite className=' relative fill-pink-500 hover:shadow-2xl transform scale-150 gap-1 items-center' /><MdFavorite className='animate-ping absolute top-0 fill-pink-500 hover:shadow-2xl transform scale-150 gap-1 items-center' /></span>&nbsp;Favorites -- -zsh- {parseInt(size.width)} x {parseInt(size.height)}</p>
        </div>
      </div>
      <div className="rounded-b-xl border" style={{ width: '100%', height: '100%', background: 'white' }}>
        {/* Popup içeriği */}
        {<Favorities width={size.width-1} height={size.height-1} onMain={onMain} />}
      </div>
    </Rnd>
  );
};

export default PopupFavorities;
