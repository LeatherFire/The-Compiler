...
const CodeEditor = ({ width, height, languge, onCodeChange, initialCode,initialLanguage }) => {
  const dispatch = useDispatch();
  const [key, setKey] = useState(0);
  const [language , setLanguage] = useState(initialLanguage.toLowerCase());
  const [code, setCode] = useState(initialCode); // Initial code from props
  ...
  useEffect(() => {
    setCode(initialCode); // Set the initial code when the component mounts or initialCode changes
  }, [initialCode]);
